...
const removeFromBasketReducer = (state: BasketState, action: PayloadAction<{ id: string}> )=> {
    const index = state.items.findIndex((item: Product) => item._id === action.payload.id)

    let newBasket = [...state.items]
    
    // Splice removes the item at the given 
    // index when the second param is 1
    if(index >= 0){
        newBasket.splice(index, 1)
    }
    else{
        console.log(`Can't remove product {id: ${action.payload.id} is not in basket!}`)
    }
